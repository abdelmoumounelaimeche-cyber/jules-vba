...
def create_workbook():
    workbook_path = os.path.join('SmartReceive', 'SmartReceive_Pro.xlsm')
    ...
    ws_dashboard.write('B4', '1. Enter Operator Name in Cell D4')
    ws_dashboard.write('B5', '2. Go to "Scanner UI" sheet to start scanning.')

    # 2. Scanner UI (Sheet-based interface)
    ...
    ws_scanner.write('B4', 'Scanner Input (Scan Here):')
    ws_scanner.write('C4', '', workbook.add_format({'bg_color': '#FFFFCC', 'border': 1})) # Input cell
    ...
    # Data Sheets
    for sheet_name, table_name, cols in [
        ...
        ws.add_table('A1:' + chr(64 + len(cols)) + '10', {'name': table_name, 'columns': [{'header': c} for c in cols]})
